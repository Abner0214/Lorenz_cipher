...
def lorenz():
   ...
   while 1:
      ...
      if wheels == []: #This test for whether the wheels need to be initialized.
         for sz in szs:
            wheels.append(cycle([choice([0, 1]) for i in range(sz)])) #This uses comprehension, append, cycle and choice.
         ...
         chi= reduce(lambda x, y: (x << 1) + y, [next(wheels[i]) for i in range(7, 12)]) #This uses next, comprehension, reduce, and lambda.
      ...
      psi= reduce(lambda x, y: (x << 1) + y, [next(wheels[i]) for i in range(5)]) #This is like the "chi=___" line above, but for wheels 0 to 4.
      ...
      if ((m1 := next(wheels[5])) == 1):#This line uses next to spin the 6th wheel and simultaneously
                 #uses expression assignment to put that next value into mu1.
         if ((m2 := next(wheels[6]))):#A similar use of next+expression assignment, but for mu2. 
            chi= reduce(lambda x, y: (x << 1) + y, [next(wheels[i]) for i in range(7, 12)]) #This is identical to the "chi=___" line up above.
...
def enc(s):
   ...
   if s.isalpha(): #Checks if s is a letter (ie, 'a', or 'A', or 'b', or ...)
      ...
      return {' ': 26, '.': 27, ',': 28, '"': 29, '?': 30, '\n': 31}.get(s, -1) #' ' -> 26, '.' -> 27, ',' -> 28, '"' -> 29,
                      #'?' -> 30, '\n' -> 32, and anything-else -> -1.
   
def dec(v):return chr(v + ord('A')) if 0 <= v <= 25 else {26: ' ', 27: '.', 28: ',', 29: '"', 30: '?', 31: '\n'}.get(v, '')#This reverses enc(). But it uses a conditional
                         #expression to implement it all on one line.

key=lorenz() # This initialize the lorenz() generator.
f=open("PA5template.py")   # This opens the file PA5template.py.
text=f.read() # This reads the entire PA5template.py into a string.

print("Encrypted:\n")
print("".join(map(lambda x: dec(x ^ next(key)), [y for c in text if (y := enc(c)) != -1])))#This uses map(), dec(), expression assignment, enc(),
...
print("\n"," - "*25,"\n\nDecrypted:\n")
encry = "".join(map(lambda x: dec(x ^ next(key)), [y for c in text if (y := enc(c)) != -1]))
start = True;next(key)
print("".join(map(lambda x: dec(x ^ next(key)), [enc(y) for y in encry if y is not None])))#This uses map, lambda, dec, ^, next, and the list of
